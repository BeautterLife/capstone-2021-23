...
def CreateMessageHtml(sender, to, subject, msgHtml, msgPlain):
    ...
    return {'raw': base64.urlsafe_b64encode(msg.as_bytes()).decode('utf8')}
...
def gmail_send(to, space, day, time, file_name):
    f = open("/home/capstone/capstone-2021-23/mysite/users/template2.html")
    template = f.read()
    f.close()
    print("sadf")
    ...
    new_str = template[start:end+12].format(space=space, day=day, time=time)
    template = template[:start] + new_str + template[end+12:]
    #print(template)

    # thumbnail
    #img_name = make_thumbnail(file_name)
    img_name = f"{file_name.split('.')[0]}.jpg"
...
#    gmail_send("plmoknijb3123@gmail.com", "1", "2", "3", "4")
